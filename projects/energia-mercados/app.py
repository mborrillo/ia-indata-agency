...
def q(sql: str, log_errors: bool = True) -> pd.DataFrame:
    """
    Ejecuta query SQL con logging de errores.
    log_errors=False para queries de verificación interna.
    """
    try:
        with get_engine().connect() as conn:
            result = pd.read_sql(text(sql), conn)
            if log_errors and result.empty:
                st.warning(f"⚠ Query devolvió 0 filas: {sql[:50]}...")
            return result
    except Exception as e:
        if log_errors:
            st.error(f"❌ Error BD: {str(e)[:100]}")
        return pd.DataFrame()
...
try:
    test_conn = q("SELECT 1 as test", log_errors=False)
    if test_conn.empty:
        st.error("⚠ Conexión a BD fallida - revisa NEON_DATABASE_URL")
        st.stop()
except Exception as e:
    st.error(f"⚠ Error de conexión: {str(e)[:80]}")
    st.stop()
...
── Índice de Coste Total — siempre visible arriba ───────────────────────────
indice = q("SELECT * FROM hosteleria.v_indice_coste")
if not indice.empty:
    r = indice.iloc[0]
    sg = str(r.get("semaforo_global","AMARILLO")).upper()
    ...
    <div class="krow k4">
      <div class="kpi">
        ...
        <div class="kval lg">{sem_icon(sg)} {sg}</div>
      </div>
      <div class="kpi">
        <div class="kacc" style="background:var(--purple)"></div>
        <div class="klbl">Luz</div>
        <div class="kval">{sem_icon(str(r.get('luz','—')))} {r.get('luz','—')}</div>
      </div>
      <div class="kpi">
        <div class="kacc" style="background:var(--amber)"></div>
        <div class="klbl">Gas</div>
        <div class="kval">{sem_icon(str(r.get('gas','—')))} {r.get('gas','—')}</div>
      </div>
      <div class="kpi">
        <div class="kacc" style="background:var(--red)"></div>
        <div class="klbl">Aceite</div>
        <div class="kval">{r.get('aceite','—')}</div>
      </div>
    </div>
    <div class="kpi" style="margin-top:12px;">
        <div class="klbl">IPC alimentación vs general</div>
        <div class="kval">{float(r.get('spread_ipc') or 0):+.2f} pts</div>
    </div>
    """, unsafe_allow_html=True)
...
════════════════════════════════════════════════════════════
TAB 4 — HISTÓRICO (CORREGIDO)
════════════════════════════════════════════════════════════
with tab4:
    # Query con LIMIT para evitar timeout si hay muchos datos
    hist = q("SELECT * FROM hosteleria.v_luz_historico ORDER BY fecha DESC LIMIT 1000")
    
    if hist.empty:
        st.warning("⚠ Sin datos en v_luz_historico")
        st.caption("Posibles causas: 1) ETL no ejecutado, 2) Vista no existe, 3) Sin datos en Bronze")
        with st.expander("🔍 Debug de tablas"):
            bronce_count = q("SELECT COUNT(*) FROM hosteleria.bronce_pvpc", log_errors=False)
            vista_exists = q("SELECT 1 FROM information_schema.views WHERE table_schema='hosteleria' AND table_name='v_luz_historico'", log_errors=False)
            st.write("📦 Bronze PVPC:", bronce_count.iloc[0,0] if not bronce_count.empty else "Error")
            st.write("👁️ Vista existe:", "✅ Sí" if not vista_exists.empty else "❌ No")
            st.write("🔗 URL BD:", DB_URL[:30] + "..." if DB_URL else "No configurada")
    else:
        st.success(f"✅ {len(hist)} registros cargados")
        ...
        with fc1:
            sel_mes = st.multiselect("Mes", meses_h, default=[], key="h_mes", placeholder="Todos los meses")
        with fc2:
            sel_wk  = st.multiselect("Semana", weeks_h, default=[], key="h_wk", placeholder="Todas las semanas")
